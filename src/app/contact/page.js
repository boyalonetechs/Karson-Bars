    ...
    <div className="min-h-screen bg-[#FFFDF0] text-[#1E1E1E] font-sans overflow-x-clip">
      ...
      <section className="max-w-7xl mx-auto px-6 py-16">
        <div className="text-center mb-12">
          <h1 className="text-2xl md:text-4xl font-bold text-[#1E1E1E] mb-2">Contact Us</h1>
          <p className="text-xs md:text-sm text-gray-500 max-w-xl mx-auto">
            Have a question or need help with an order? Send us a message and we will reply on WhatsApp.
          </p>
        </div>

        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 items-start">
          {/* Contact Form */}
          <form onSubmit={handleSubmit} className="bg-white/60 border border-yellow-200/50 rounded-3xl p-8 shadow-sm space-y-4">
            <div>
          ...
          <div className="space-y-4">
            {[
              ...
              <div key={idx} className="bg-[#801B1B] text-white rounded-2xl p-6 flex items-start space-x-4 shadow-md">
                ...
                <div>
                  ...
                  <p className="text-xs text-red-100 leading-relaxed">{item.text}</p>
                  {item.action}
                </div>
              </div>
            ))}
